...
GetInstanceNumberFromMcDir(mcdir) {
  ...
  num := -1
  if (mcdir == "" || mcdir == ".minecraft" || mcdir == ".minecraft\" || mcdir == ".minecraft/") {
    Reload ; Misread something
  }
...
SwitchInstance(idx, skipBg:=false, from:=-1) {
  if (idx <= instances) {
    ...
    if (coop) {
      ...
      sleep, %guiDelay%
      ControlSend, ahk_parent, {Blind}{Tab 4}{Enter}{Tab}{Enter}, ahk_pid %pid%
    }
...
Setup() {
    ...
    setupCheck := False
    SetTitleMatchMode, 1
    mcTitle = Minecraft %version% - Instance %i%
    if (!WinExist(mcTitle)) {
		  setupCheck := True
	  }
    WinSetTitle, ahk_pid %pid%, , Minecraft %version% - Instance %i%
    if (borderless) {
      WinRestore, Minecraft %version% - Instance %i%
      WinSet, Style, -0xC40000, Minecraft %version% - Instance %i%
      WinMove, Minecraft %version% - Instance %i%,,0,0,%A_ScreenWidth%,%A_ScreenHeight%
      newHeight := Floor(A_ScreenHeight)
      WinMove, Minecraft %version% - Instance %i%,,0,0,%A_ScreenWidth%,%newHeight%
    }
    if (setup && setupCheck) {
      RDPresses := renderDistance - 2
      FPSPresses := (260 - FPScap)/5
      ControlSend, ahk_parent, {Blind}{Esc}, Minecraft %version% - Instance %i%
      Sleep %guiDelay%
      ControlSend, ahk_parent, {Blind}{Tab 4}{Enter}, Minecraft %version% - Instance %i%
      Sleep %guiDelay%
      ControlSend, ahk_parent, {Blind}{Tab 7}{Enter}, Minecraft %version% - Instance %i%
      Sleep %guiDelay%
      ControlSend, ahk_parent, {Blind}{Tab 2}, Minecraft %version% - Instance %i%
      ControlSend, ahk_parent, {Blind}{Left 30}, Minecraft %version% - Instance %i%
      ControlSend, ahk_parent, {Blind}{Right %RDPresses%}, Minecraft %version% - Instance %i%
      ControlSend, ahk_parent, {Blind}{Tab 2}, Minecraft %version% - Instance %i%
      ControlSend, ahk_parent, {Blind}{Right 52}, Minecraft %version% - Instance %i%
      ControlSend, ahk_parent, {Blind}{Left %FPSPresses%}, Minecraft %version% - Instance %i%
      Sleep %guiDelay%
      ControlSend, ahk_parent, {Blind}{Esc}, Minecraft %version% - Instance %i%
      Sleep %guiDelay%
      ControlSend, ahk_parent, {Blind}{Tab 6}, Minecraft %version% - Instance %i%
      ResetInstance(i)
      Sleep 1000
    }
  }
...
ResetSettings(pid, rd) {
  if (rd) {
    ...
    ControlSend, ahk_parent, {Blind}{Esc}, ahk_pid %pid%
    sleep, %guiDelay%
    ControlSend, ahk_parent, {Blind}{Tab 3}{Enter}, ahk_pid %pid%
    sleep, %guiDelay%
    ControlSend, ahk_parent, {Blind}{Tab 8}{Enter}, ahk_pid %pid%
    sleep, %guiDelay%
    ControlSend, ahk_parent, {Blind}{Tab 2}{Left 32}{Right %RDPresses%}{Esc}, ahk_pid %pid%
    sleep %postSettingsDelay%
  }
}
